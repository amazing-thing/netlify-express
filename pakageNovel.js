...
function translateNovel(response) {
  // 从哪个文件夹读取数据
  const dirPath = path.resolve(__dirname, './file', `./${response.bookName + response.uid}`)

  // 新章节目录
  const newFileDir = path.resolve(__dirname, './newFiles', `./${response.bookName + response.uid}`)
  // 初始化文件夹
